...
int main(int argc, char **argv) {
    ...
    sin.sin_addr.s_addr = INADDR_ANY; // all chanels this machine listens e.g. wifi...
    ...
    while (1)
    {
        ...
        if (select_retval > 0) /* at least one file descriptor is ready */
        {
            if (FD_ISSET(sock, &read_set)) /* check the server socket */
            {
                 ...
                 */
                if (fcntl (new_sock, F_SETFL, O_NONBLOCK) < 0) // return control back to server if blocked at some client
                {
