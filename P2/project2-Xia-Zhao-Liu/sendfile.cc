...
void handleTimeoutPkt(int windowStart, vector<char*> my_packets, sockaddr_in sin, int sock, struct timeval lastACKtv) {
    ...
    if (isTimeout(windowStart, my_packets)) {
        ...
        if (restv.tv_sec > 5) {
            ...
            cout << "Haven't received any reply from receiver in 5 seconds. Receiver might be off...program exits now" << endl;
            cout << "[completed]" << endl;
            exit(0);
        }
        if (getType(my_packets.at(0)) == 3) {
            ...
            if (lastPktRetry <= 100) {
                ...
                cout << "The last packet has been resent for 100 times. Give up retrying...program exits now" << endl;
                cout << "[completed]" << endl;
                exit(0);
            }
        }
